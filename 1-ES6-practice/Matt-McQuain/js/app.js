...
function displayPics(){
  ...
  // I think that our code threw us an error when we changed the variable declaration from var to let because we called outside of our code block. 
  console.log(viewed)
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`{$event.target.id} has ${allProducts[i].votes} vote in ${allProducts[i].views} views`);
    }
  }
